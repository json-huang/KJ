...
public sealed class LoginViewModel : BindableBase
{
    ...
    private async Task ExecuteSignInAsync()
    {
        ...
        if (RememberEmail)
            _credentialStore.SaveRememberedEmail(normalizedEmail);
        else
            _credentialStore.ClearRememberedEmail();

        if (StaySignedIn)
            _credentialStore.SaveStaySignedIn(normalizedEmail, Password);
        else
            _credentialStore.ClearStaySignedIn();

        _sessionState.SetSignedIn(normalizedEmail);
        _navigator.GoMain();
    }
}
